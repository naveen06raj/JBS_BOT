-- Fix for "column sl.is_active does not exist" error in fn_get_leads_with_pagination function
-- The column name should be 'isactive' (no underscore) based on SalesLead model

-- Drop existing function to replace it
DROP FUNCTION IF EXISTS fn_get_leads_with_pagination;

-- Create function with correct column name (isactive instead of is_active)
...
BEGIN
    ...
    v_order_clause := ' ORDER BY ';        CASE p_order_by
        WHEN 'date_created' THEN v_order_clause := v_order_clause || 'sl.created_date ';
        ...
        ELSE v_order_clause := v_order_clause || 'sl.date_created ';
    END CASE;
    ...
    -- Main query to get paginated results - Fix column name from is_active to isactive
    v_main_query := '
        SELECT 
            ' || v_total_records || ' AS "TotalRecords",
            sl.id AS "Id",
            sl.user_created AS "UserCreated",            sl.created_date AS "DateCreated",
            ...
            sl.status AS "Status",
            sl.score AS "Score",
            sl.isactive AS "IsActive", -- Fixed: changed is_active to isactive
