-- Fix the PostgreSQL error "relation 'territories' does not exist"
-- Drop the existing functions that reference the incorrect table
DROP FUNCTION IF EXISTS fn_get_leads_with_pagination;
DROP FUNCTION IF EXISTS fn_get_sales_lead_by_id;

-- Create the updated functions with the correct table name 'sales_territories' and column name 'name'
...
BEGIN
    ...
    IF p_territories IS NOT NULL AND array_length(p_territories, 1) > 0 THEN
        v_where_clause := v_where_clause || ' AND t.name = ANY($3)';
    END IF;
    ...
    v_main_query := '
        SELECT 
            ...
            t.name AS "TerritoryName"        FROM sales_leads sl        LEFT JOIN sales_territories t ON sl.territory = t.name
