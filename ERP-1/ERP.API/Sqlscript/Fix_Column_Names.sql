-- Fix for "column sl.created_by does not exist" error in fn_get_leads_with_pagination function

-- Drop existing function to replace it
DROP FUNCTION IF EXISTS fn_get_leads_with_pagination;

-- Create function with correct column names based on the BaseEntity class:
-- user_created instead of created_by
-- date_created instead of created_date
-- user_updated instead of updated_by
-- date_updated instead of updated_date
...
BEGIN
    ...
    v_main_query := '
        SELECT 
            ...
            sl.street AS "Street",            sl.landmark AS "Landmark",
