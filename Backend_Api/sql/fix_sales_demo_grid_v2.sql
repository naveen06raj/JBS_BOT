...
BEGIN
    ...
    SELECT ARRAY(
        SELECT value 
        FROM json_array_elements_text(COALESCE(p_request->'CustomerNames', '[]'::json))
        WHERE value != 'string'
    ) INTO v_customer_names;
    
    SELECT ARRAY(
        SELECT value 
        FROM json_array_elements_text(COALESCE(p_request->'Statuses', '[]'::json))
        WHERE value != 'string'
    ) INTO v_statuses;
    
    SELECT ARRAY(
        SELECT value 
        FROM json_array_elements_text(COALESCE(p_request->'DemoApproaches', '[]'::json))
        WHERE value != 'string'
    ) INTO v_demo_approaches;
    
    SELECT ARRAY(
        SELECT value 
        FROM json_array_elements_text(COALESCE(p_request->'DemoOutcomes', '[]'::json))
        WHERE value != 'string'
