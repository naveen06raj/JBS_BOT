-- Fix for "relation 'zones' does not exist" error in fn_get_leads_with_pagination function

-- Drop existing function to replace it
DROP FUNCTION IF EXISTS fn_get_leads_with_pagination;

-- Create function without dependency on zones
...
BEGIN
    ...
    -- Count query to get total records - REMOVED zones table reference
    v_count_query := '
        SELECT COUNT(*) 
        FROM sales_leads sl
    ...
    -- Main query to get paginated results - REMOVED zones table reference
    v_main_query := '
        SELECT 
